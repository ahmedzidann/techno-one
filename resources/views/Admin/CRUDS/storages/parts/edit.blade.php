...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('storages.update',$row->id)}}">
    @csrf
    @method('PUT')
    <div class="row g-4">
        ...
        <div class="d-flex flex-column mb-7 fv-row col-sm-6">
            ...
            <select id="branch_id" name="branch_id" class="form-control">
                ...
                @foreach($branches as $branch)
                    <option @if($row->branch_id==$branch->id)  selected @endif value="{{$branch->id}}"> {{$branch->title}}</option>
                @endforeach
            </select>
