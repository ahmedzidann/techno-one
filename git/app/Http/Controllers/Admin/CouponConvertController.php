...
// زرار التعديل
if (auth('admin')->user()?->can('تعديل الكوبونات')) {
    $buttons .= '
...
// زرار الحذف
if (auth('admin')->user()?->can('حذف الكوبونات')) {
    $buttons .= '
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'amount'       => 'required|numeric|min:1',
             'invoice_value' => 'required|integer',
             'date' => 'required',
            ...
            $data_insert['to_user_id'] = 0;
             ...
             $data_insert['publisher'] = auth('admin')->id();
              ...
              $data_insert['date'] = $data['date'] ;
                $data_insert['invoice_value'] = $data['invoice_value'] ;
    ...
    public function CouponStatus($status,Request $request)
    {
        if ($request->ajax()) {
            ...
            return DataTables::of($rows)
              ...
              if (auth('admin')->check() && auth('admin')->user()->can('الاجراء علي التحويل')) {
