...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            ...
            @include('Admin.CRUDS.headBackPurchases.parts.editForm')
    ...
    @section('js')
        ...
        <script>
            ...
            $(document).on('change', '#supplier_id', function() {
                let supplier = $('#supplier_id').val(); // Use the correct selector with `#` for ID
                if (supplier) {
                    let url = '{{ route('admin.getPurchasesForSupplier', ['supplier_id' => ':id']) }}'.replace(':id',
                        supplier);
                    $("#purchase_id").select2({
                        placeholder: 'Channel...',
                        // width: '350px',  
            ...
            $(document).on('change', '#purchase_id', function(e) {
                ...
                if (purchase_id) {
                    let url = '{{ route('admin.head-back-purchases.invoice-details', ['purchase_id' => ':id']) }}'
                        .replace(':id', purchase_id);
                    $.ajax({
                        type: 'GET',
                        url: url,
            ...
            (function() {
                ...
                if (purchase_id) {
                    ...
                    let url = "{{ route('admin.head-back-purchases.invoice-details-edit', ['purchases_id' => ':id']) }}"
                        .replace(':id', purchase_id);
        ...
        <script>
            @foreach (\App\Models\HeadBackPurchasesDetails::where('head_back_purchases_id', $row->id)->get() as $key => $pivot)
